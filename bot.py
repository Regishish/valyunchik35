...
async def handle_user_response(message: types.Message):
    if message.from_user.id in USER_IDS:
        ...
        if state < len(QUESTS):
            ...
            # Похвалы по индексам квестов
compliments = {
    0: "🥐 Завтрак выглядит так, что Michelin бы позавидовал! Красиво, сытно — ты задал дню правильный вкус.",
    1: "📞 Отлично, спасибо! Это было важно. Теперь у нас с тобой +100 к карме и маминому расположению 💕",
    2: "🎂 Это волшебный торт, волшебный день — и желание? Ну оно просто обязано сбыться!",
    3: "🏖️ На этом пляже ты как и на любом другом — статный Апполон в своей стихии!",
    4: "🍹 Отличный выбор, Чин-чин! Пусть в этом бокале будет вся лёгкость и радость дня!",
    5: "🕺 Ты — звезда танцпола, даже если танцпол — это балкон! Настроение на максимум?",
    6: "🍽️ Я за любой твой выбор, особенно если мы идём туда вместе ❤️"
}
...
async def handle_user_response(message: types.Message):
    ...
    if user_id in USER_IDS:
        ...
        if state < len(QUESTS):
            # Отправляем похвалу по номеру задания
            compliment = compliments.get(state, "💪 Молодец!")
